...
const options = {
  ...
  selectrors: {
    product_link: '#catalog .item-all-title a',
    files: '.document-item__title',
    product_name: '.body_text h1',
    sku: '.article',
    main_pic: '.catalog-detail-images',
    description: '.description',
    table_options: '.catalog-detail-property'
  }
};

const pages_category = [
  'https://weicon.su/catalog/silikonovye-klei-germetiki/'
];
...
const getLinksProducts = async (page) => {
  for (let item of pages_category) {
    ...
    $(options.selectrors.product_link).each((i, elem) => {
      products_links.push('https://weicon.su' + $(elem).attr('href'));
    });
